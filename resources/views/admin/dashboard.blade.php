...
@section('content')
    ...
    <div class="mt-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach ($totalTabunganPerKelas as $kelasId => $total)
                <div class="bg-white shadow p-6 rounded-lg border border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-700 mb-1">Kelas {{ $namaKelasById[$kelasId] ?? '-' }}</h3>

                    <p class="text-sm text-gray-500 mb-1">Jumlah siswa aktif:
                        <span class="font-medium text-gray-800">{{ $jumlahSiswaPerKelas[$kelasId] ?? 0 }}</span>
                    </p>
                    <p class="text-sm text-gray-500 mb-1">Jumlah transaksi:
                        <span class="font-medium text-gray-800">{{ $jumlahTransaksiPerKelas[$kelasId] ?? 0 }}</span>
                    </p>
                    <p class="text-base font-bold text-green-600">Total Tabungan:
                        Rp{{ number_format($total, 0, ',', '.') }}
                    </p>
                    <p class="text-base text-blue-600 font-semibold">Total Setoran:
                        Rp{{ number_format($totalSetoranPerKelas[$kelasId] ?? 0, 0, ',', '.') }}
                    </p>
                    <p class="text-base text-red-600 font-semibold">Total Penarikan:
                        Rp{{ number_format($totalPenarikanPerKelas[$kelasId] ?? 0, 0, ',', '.') }}
                    </p>
                    <p class="text-base font-semibold text-gray-700">Rata-rata Tabungan (per Kelas):
                        Rp{{ number_format($rataRataPerKelas[$kelasId] ?? 0, 0, ',', '.') }}
                    </p>

                    <div class="mt-4">
                        <p class="text-sm text-gray-500">Disetor oleh:
                            @if (!empty($setoransByKelas[$kelasId]))
                                <span class="font-medium text-gray-800">
                                    {{ implode(', ', $setoransByKelas[$kelasId]) }}
                                </span>
                            @else
                                <span class="font-medium text-gray-800">Tidak ada setoran</span>
                            @endif
                        </p>

                        <p class="text-sm text-gray-500">Ditarik oleh:
                            @if (!empty($penarikansByKelas[$kelasId]))
                                <span class="font-medium text-gray-800">
                                    {{ implode(', ', $penarikansByKelas[$kelasId]) }}
                                </span>
                            @else
                                <span class="font-medium text-gray-800">Tidak ada penarikan</span>
                            @endif
                        </p>
                    </div>
                </div>
